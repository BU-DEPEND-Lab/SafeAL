...
class mountaincar(grids, object):
    ...
    def set_target(self):
        ...
        for s in self.M.S[:-2]:
            coords = self.index_to_coord(s)
            if (coords[0] >= (18 - 1) * (self.grids[0] - 2)/18.0):
                self.M.targets.append(s)
    ...
    def read_adv(self, path = './mdp.adv'):
        file = open(path, 'r')
        for line in file.readlines():
            if re.search('(?<=Strategy:)', line) is not None:
                self.policy = np.zeros([322, 200])
                continue
